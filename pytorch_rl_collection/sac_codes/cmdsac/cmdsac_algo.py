...
def cmdsac_algorithm(agent, env, args, np, use_encoder, output_path):
    ...
    if not args.just_run_test_target:
        ...
        while agent.steps < args.total_num_steps:
            ...
            ## + Sample varpi
            varpi = agent.sample_varpis(kappas=I)#np.copy(varpis)#(size=1)#
                ...
                for k in range(args.number_of_updates):
                    ...
                    for mb_s, mb_a, mb_r, mb_ns, mb_m, mb_p, mb_v in agent.rmset_sample_minibatch(batch_size=args.batch_size):
                        if minibatch_states is None:
                            ...
                            minibatch_masks, minibatch_kappas, HER_varpis = mb_s, mb_a, mb_r, mb_ns, mb_m, mb_p, mb_v
                        else:
                    ...
                    y = agent.get_td_target(states=minibatch_states, rewards=minibatch_rewards, next_states=minibatch_new_states, masks=minibatch_masks, kappas=minibatch_kappas, her_varpis=HER_varpis)
                    ## + Update both critics
            ...
            if i_episode % 10 == 0:
                ...
                print("--------------------------------")
                #print("++ ", np.round(nominal_varpi, 4))
...
def run_cmdsac_algorithm(args):
    ...
    from pytorch_rl_collection.sac_codes.cmdsac.cmdsac_agent import CMDSAC_AGENT
    ...
    agent = CMDSAC_AGENT(states_dim, actions_dim, domains_dim, args, OptimCls=TAdam,
                       ReplayBufferCls=MDReplayMemorySet, value_range=value_range,
                       add_noise=add_observation_noise,
                       model_type=model_type)

    # - Run the CMDSAC algorithm
    date = datetime.now()
    ...
    ###
    cmdsac_algorithm(agent, env, args, np, use_encoder, output_path)
    if not args.just_run_test_target:
