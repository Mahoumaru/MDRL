...
class DRSAC_RNN_AGENT:
    def __init__(self, states_dim, actions_dim, args, OptimCls=torch.optim.Adam,
        ...
        self.truncated_mem = args.truncated_mem
        self.max_sequence_length = args.max_sequence_length
        self.adjustment_percentage = args.adjustment_percentage
        if not (0. <= self.adjustment_percentage <= 1.):
            print("++ Got an invalid adjustment_percentage = {}. It will automatically\
                be cropped to lie within the range [0, 1]."
            )
            self.adjustment_percentage = np.clip(self.adjustment_percentage, a_min=0., a_max=1.)
        ...
        if self.model_type == "lstm":
            ...
            print("++ {} is not a valid RNN DRSAC type".format(self.model_type))
    ...
    def initialize_replay_buffer(self, max_episode_steps):
        if self.truncated_mem:
            self.replay_memory = self.ReplayBufferCls(capacity=self.rmsize, seed=self.seed,
                max_sequence_length=self.max_sequence_length, max_episode_steps=max_episode_steps,
                adjust_sampling_lengths=True, adjustment_percentage=self.adjustment_percentage,
    ...
    def store_whole_episode(self, episode):
        if self.truncated_mem:
            ...
            self.replay_memory.push(episode_state, episode_action, episode_last_action, \
    ...
    def sample_minibatch(self, batch_size):
        ...
        if len(self.replay_memory) > batch_size:
            ...
            if self.truncated_mem:
                ...
                hidden_dim = self.net_cfg['hidden_layers'][0]
                if self.model_type == "gru":
                    hidden_in = torch.zeros([1, 1, hidden_dim], dtype=torch.float).repeat(1, batch_size, 1).to(self.device)
                else:
                    hidden_in = (
                        torch.zeros([1, 1, hidden_dim], dtype=torch.float).repeat(1, batch_size, 1).to(self.device), \
                        torch.zeros([1, 1, hidden_dim], dtype=torch.float).repeat(1, batch_size, 1).to(self.device)
                    )
                ######
                with torch.no_grad():
                    _, _, hidden_out = self.actor(state_batch, last_action_batch, hidden_in, lengths)
            else:
