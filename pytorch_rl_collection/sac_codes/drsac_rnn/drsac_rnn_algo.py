...
def drsac_rnn_algorithm(agent, env, args, output_path, eval_env, np):
    ...
    agent.initialize_replay_buffer(max_episode_steps=env._max_episode_steps)
    ...
    while agent.steps < args.total_num_steps:
        ...
        for t in count(0):
            ...
            new_state, reward, done = new_state.reshape(-1), reward[0], done[0]
            ###
        ...
        if args.truncated_mem:
            ...
            episode_last_actions = np.stack(episode_last_actions)
            episode_rewards = np.stack(episode_rewards)
            episode_next_states = np.stack(episode_next_states)
            episode_dones = np.stack(episode_dones)
            agent.store_whole_episode((episode_states, episode_actions, episode_last_actions, \
...
###########################################
def run_drsac_rnn_algorithm(args):
    import robel
    import custom_gym
    ...
    if args.truncated_mem:
        from pytorch_rl_collection.replay_buffers.replay_memory import TruncatedReplayMemoryLSTM as ReplayMemory
    elif args.rnn_type in ["lstm", "lstm2"]:
        ...
        print("++ {} is not a valid RNN DRSAC type".format(args.rnn_type))
        raise NotImplementedError
    from pytorch_rl_collection.sac_codes.drsac_rnn.drsac_rnn_agent import DRSAC_RNN_AGENT
    ...
    # - Initialize the environment
    env = MDEnvSet(env_name=args.env_name, multi_dim=args.multi_dim, n_regions=1, seed=args.seed, use_encoder=use_encoder, add_noise=False)
    #####
    ...
    agent = DRSAC_RNN_AGENT(states_dim, actions_dim, args, OptimCls=TAdam,
                       ReplayBufferCls=ReplayMemory, value_range=value_range,
                       add_noise=add_observation_noise, OptimArgs={"beta_dof": 1., "k_dof": np.inf})
    ...
    output_path = "./trained_agents/{}/{}_{}/{}/".format(
        algo + "_{}".format(args.rnn_type) + ("_trunc_mem" + str(args.adjustment_percentage).replace(".", "p") if args.truncated_mem else ""),
    ...
    drsac_rnn_algorithm(agent, env, args, output_path, eval_env, np)
    ...
    run_test_target(actor=agent.actor, output_path=output_path, env_name=args.env_name, algo_name=args.algo,
            ...
            seed=1234, n_runs=50, cuda=args.cuda, cuda_id=args.cuda_id, rnn_type=args.rnn_type)
