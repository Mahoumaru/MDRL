...
class  UMDSAC_AGENT:
    ...
    def initialize(self, use_solver):
        global USE_SOLVER
        ...
        self.initialize_solver(use_solver)
    ...
    def update_utopia_varpis(self, next_states, kappas, utopia_varpis, replay_memory_indices):
        if next_states is not None:
            varpi_size = self.varpi_size
            print("+ Before update utopia_varpis: ", utopia_varpis[0], utopia_varpis[-1])
            mu, sig = torch.split(utopia_varpis, varpi_size//2, dim=-1)
            wrapped_mu = (mu - self.min_rel_param) / (self.max_rel_param - self.min_rel_param)
            wrapped_sig = sig / torch.min((self.max_rel_param - mu)/np.sqrt(3.), (mu - self.min_rel_param)/np.sqrt(3.))
            wrapped_sig = wrapped_sig.clip(min=1e-8, max=1.-1e-8)

            wrapped_mu.requires_grad_()
            wrapped_sig.requires_grad_()

            mu = self.min_rel_param + wrapped_mu * (self.max_rel_param - self.min_rel_param)
            sig = (wrapped_sig * (self.max_rel_param - self.min_rel_param) / (2.*np.sqrt(3.))) * (1. - (1. - 2.*wrapped_mu).abs())

            utopia_varpis = torch.cat((mu, sig), dim=-1)
            _, _, next_states_actions = self.actor(torch.cat(
              (next_states, utopia_varpis), dim=-1
            ))
            ...
            ## Q1
            q1_values = self.critic1([ aug_next_states, next_states_actions ])
            ## Q2
            q2_values = self.critic2([ aug_next_states, next_states_actions ])
            ## min(Q1, Q2)
            min_qf_target = - torch.min(q1_values, q2_values).mean()
            min_qf_target.backward()

            mirror_descent_update([wrapped_mu, wrapped_sig], lr=1e-1)
            #print(wrapped_mu.grad)
            ...
            ## Update the utopia varpi in the replay memory
            self.replay_memory.update_utopia_varpis(utopia_varpis.cpu().numpy(), replay_memory_indices)
            ###
            print("+ After update utopia_varpis: ", utopia_varpis[0], utopia_varpis[-1])
            print(jkll)
            return utopia_varpis
        else:
            return None

    def get_td_target(self, states, actions, rewards, next_states, masks, kappas, her_varpis, varpis=None):
        if self.continuous:
            return self._get_td_target_continuous(states, actions, rewards, next_states, masks, kappas, her_varpis, varpis)
        else:
            raise NotImplementedError

    def _get_td_target_continuous(self, states, actions, rewards, next_states, masks, kappas, her_varpis, varpis=None):
    ...
    def update_critics(self, td_target, states, actions, kappas, her_varpis):
        if self.continuous:
            self._update_critics_continuous(td_target, states, actions, kappas, her_varpis)
        else:
            raise NotImplementedError

    def _update_critics_continuous(self, td_target, states, actions, kappas, her_varpis):
    ...
    def update_solver(self, states, kappas):
        if self.continuous:
            self._update_solver_continuous(states, kappas)
        else:
            raise NotImplementedError

    def _update_solver_continuous(self, states, kappas):
        if states is not None and USE_SOLVER:
